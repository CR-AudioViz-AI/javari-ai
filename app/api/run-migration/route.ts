...
export async function GET() {
  try {
    console.log('[Migration] Starting table creation via Supabase REST API...');

    // SQL for all 3 missing tables in one statement
    const migrationSQL = `
    ...
    console.log('[Migration] Executing SQL via Supabase REST API...');
    ...
    console.log(`[Migration] Response body: ${responseText.substring(0, 500)}`);

    if (!response.ok) {
      throw new Error(`Supabase API error: ${response.status} - ${responseText}`);
    }

    console.log('[Migration] ✅ All 3 tables created successfully');

    return NextResponse.json({
      ok: true,
      message: '✅ All 3 missing tables created',
      tables: ['user_accounts', 'ai_usage_logs', 'router_analytics'],
