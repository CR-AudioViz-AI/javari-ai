...
// Purpose: One-shot endpoint to populate roadmap_tasks from MASTER_ROADMAP_v3.1
// Protected by x-ingest-secret header. Safe to call repeatedly — skips duplicates.
// Date: 2026-03-06
...
// ─── Canonical Roadmap — MASTER_ROADMAP_v3.1 ─────────────────────────────────
const ROADMAP: Array<{
  phase: string;
  ...
  },
  {
    phase: "payments",
    tasks: [
      { title: "Complete CreditsOS Global Billing Currency Layer", description: "Implement the full CreditsOS specification: dollar-to-credit mapping table, credit purchase flow (Stripe + PayPal), usage deduction per AI action, low-balance alerts, and admin credit grant endpoint." },
  ...
  },
  {
    ...
    tasks: [
      ...
      { title: "Build AvatarOS Face and Voice Generation", description: "Implement AvatarOS core: AI face generation with style controls, voice cloning/synthesis, character memory persistence, branding package export (PNG, SVG, video loop), and behavioral tuning sliders." },
  ...
  },
  {
    ...
    tasks: [
      ...
      { title: "Build Javari Books Chapter Planner and Export", description: "Implement Javari Books: chapter planner with AI outline generation, voiceover export via StudioOS, PDF/ePub generator, AI cover designer, and marketplace publishing integration." },
      ...
      { title: "Build Market Crawler Competitive Intelligence Engine", description: "Build the Market Crawler: automated competitor crawling pipeline, pricing intelligence extraction, feature diff engine, opportunity detection algorithm, and trend ingestion from RSS/social feeds." },
...
export async function POST(req: NextRequest) {
  ...
  const expected = process.env.INGEST_SECRET ?? "crai-roadmap-ingest-2026";
  if (secret !== expected) {
  ...
  try {
    ...
    // Build rows in phase priority order with sequential global index for stable IDs
    ...
    for (const row of toInsert) {
      ...
      if (insertErr) {
        if (insertErr.message.includes("duplicate key") || insertErr.code === "23505") {
          // Race condition — treat as skip
        } else {
          failed.push(`${row.id}: ${insertErr.message}`);
        }
      } else {
        insertLogs.push(row.id);
    ...
    console.log(`[ingest-roadmap] phases_detected=${phases.length} inserted=${insertLogs.length} skipped=${skipped} failed=${failed.length}`);
    ...
    console.error("[ingest-roadmap] ❌ Error:", message);
    return NextResponse.json({ ok: false, error: message }, { status: 500 });
  }
}

// GET — status check (no auth required)
export async function GET() {
  ...
  return NextResponse.json({
    ok: true,
    roadmap_tasks_in_db: count ?? 0,
    message: "POST with x-ingest-secret header to run ingestion",
  });
}
