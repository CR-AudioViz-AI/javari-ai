  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json({
        ok: false,
        status: res.status,
        error: (data as { error?: { message?: string } }).error?.message ?? JSON.stringify(data).slice(0, 200),
      }, { status: 500 });
    }

    const link = data.link as { productionBranch?: string } | undefined;
    console.log("[vercel-config] productionBranch updated to:", link?.productionBranch);

    return NextResponse.json({
      ok: true,
      productionBranch: link?.productionBranch ?? "unknown",
...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, productionBranch: data.link?.productionBranch ?? "unknown" });
}
