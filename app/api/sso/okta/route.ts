...
export const GET = safeHandler(async (req: NextRequest) => {
  ...
  // Okta not configured — return 307 redirect to login with friendly error
  if (!process.env.OKTA_DOMAIN) {
    return NextResponse.redirect(`${appUrl}/auth/login?error=okta_not_configured`);
  }
  ...
  if (error) {
    return NextResponse.redirect(`${appUrl}/auth/login?error=okta_sso_denied`);
  }
    ...
    if (url && sbKey) {
      const res = await fetch(`${url}/auth/v1/admin/users`, {
        ...
        body: JSON.stringify({
          ...
          user_metadata: { name: userInfo.name, sso_provider: "okta", okta_sub: userInfo.sub, okta_groups: userInfo.groups },
        }),
      });
