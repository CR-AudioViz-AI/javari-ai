  ...
  return await safeAsync(
    async () => {
      ...
      return NextResponse.json({ 
        success: true, 
        message: 'code-review endpoint',
        data: []
      });
    },
    { file: 'advanced/code-review/route.ts', function: 'GET' },
...
export async function POST(request: NextRequest) {
  return await safeAsync(
    async () => {
      ...
      return NextResponse.json({ 
        success: true,
        message: 'code-review created' 
      });
    },
    { file: 'advanced/code-review/route.ts', function: 'POST' },
    NextResponse.json({ error: 'Internal error' }, { status: 500 })
  ) || NextResponse.json({ error: 'Unexpected error' }, { status: 500 });
}
