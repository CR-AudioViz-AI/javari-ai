  ...
  return await safeAsync(
    async () => {
      ...
      return NextResponse.json({ 
        success: true, 
        message: 'ai-insights endpoint',
        data: []
      });
    },
    { file: 'advanced/ai-insights/route.ts', function: 'GET' },
...
export async function POST(request: NextRequest) {
  return await safeAsync(
    async () => {
      ...
      return NextResponse.json({ 
        success: true,
        message: 'ai-insights created' 
      });
    },
    { file: 'advanced/ai-insights/route.ts', function: 'POST' },
    NextResponse.json({ error: 'Internal error' }, { status: 500 })
  ) || NextResponse.json({ error: 'Unexpected error' }, { status: 500 });
}
